...
async def cb_sender_menu(cb: CallbackQuery):
    if not admin(cb.from_user.id): return
    await cb.message.edit_text(
        "📢 *Рассыльщик*\n\n"
        "• *Юзербот* — пишет от имени вашего аккаунта (Pyrogram)\n"
        "• *Бот* — пишет от имени добавленного бота (Bot API)\n\n"
        "💡 Переменные в тексте: `{name}` `{username}` `{last_name}` `{full_name}`",
...
async def cb_send_mode(cb: CallbackQuery, state: FSMContext):
    ...
    label = "юзербот (Pyrogram)" if mode == "userbot" else "бота (Bot API)"
    await cb.message.edit_text(
...
async def handle_sender_input(message: Message, state: FSMContext):
    ...
    if message.document and message.document.file_name.endswith(".csv"):
        ...
        path = os.path.join(config.EXPORTS_DIR, f"snd_{message.from_user.id}.csv")
        ...
        await message.answer(
            "❌ Отправьте CSV файл или напишите `last`.",
            parse_mode="Markdown",
        )
        return
...
@router.message(SenderState.wait_message)
async def handle_sender_text(message: Message, state: FSMContext, session_manager: SessionManager):
    if not admin(message.from_user.id): return
    ...
    delay_raw = await database.get_setting("delay_send", "3-10")
    try:
    ...
    try:
        if mode == "userbot":
            ...
            if not client:
                await msg.edit_text(
                    "❌ Нет активных аккаунтов. Добавьте в разделе 👥 Аккаунты.",
                    reply_markup=back_kb("sender_menu"),
                )
                tasks.done_task(stop)
                return
            stats = await S.via_userbot(client, users, template, dmin, dmax, on_progress=prog, stop=stop)
        else:
            ...
            if not bots:
                ...
                tasks.done_task(stop)
                return
            stats = await S.via_bot(bots[0]["token"], users, template, dmin, dmax, on_progress=prog, stop=stop)
    finally:
        tasks.done_task(stop)
